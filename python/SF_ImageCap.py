...
class stardew_fisher:
    ...
    def capture_screen(self):
        ...
        time.sleep(1)
        while True:
            box_dims = (800, 305, 840, 855)
            screen = np.array(ImageGrab.grab(bbox=box_dims)) #capture the window (wrote script to resize window)
            screen = cv2.cvtColor(screen, cv2.COLOR_BGR2RGB)
            fish_row = fi.locate_fish(screen)
            start_col = 3
            end_col = 38
            cv2.rectangle(screen, (start_col, fish_row), (end_col, fish_row+27), [0, 255, 255], 2)
